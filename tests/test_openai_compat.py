...
def test_resolve_openai_builtin_voice() -> None:
    assert resolve_openai_voice("alloy", native_voices=set()) == "en_female"
    assert resolve_openai_voice("onyx", native_voices=set()) == "en_male"


def test_resolve_openai_logical_voice() -> None:
    assert resolve_openai_voice("zh_female", native_voices=set()) == "zh_female"


def test_resolve_openai_native_voice() -> None:
    assert resolve_openai_voice("af_bella", native_voices={"af_bella"}) == "af_bella"


def test_resolve_openai_unknown_voice() -> None:
    with pytest.raises(ValueError, match="Unknown voice"):
        resolve_openai_voice("missing", native_voices=set())
